...
import { RenderView, Scene, SceneView } from "@houseki-engine/scene";
...
export function selectTarget(
  ...
  for (const event of pointerUp) {
    ...
    for (const [renderView, sceneView] of views) {
      ...
      for (const sceneEnt of scenes) {
        if (sceneEnt.id === sceneView.active) {
          rootObject = renderStore.scenes.get(sceneEnt.id);
          break;
        }
      }
