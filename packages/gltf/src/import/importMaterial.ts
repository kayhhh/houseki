...
export function importMaterial(
...

  {
    ...
    const textureId = createTexture(gltfTexture, commands);
    if (textureId) {
...
function createTexture(gltfTexture: GltfTexture | null, commands: Commands) {
  ...
  asset.data = Array.from(imageData);
